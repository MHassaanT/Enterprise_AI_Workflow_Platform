...
async def verify_email_with_emailable(email: str) -> Dict[str, Any]:
    """
    Performs real-time email verification via Emailable API.
    Endpoint: https://api.emailable.com/v1/verify
    """
    import os
    import httpx

    api_key = os.getenv("EMAILABLE_API_KEY", "").strip()
    if not api_key:
        try:
            from config import settings
            api_key = (settings.EMAILABLE_API_KEY or "").strip()
        except Exception:
            pass
...
async def verify_email(email: str, source: str = "unknown", tenant_id: str = "00000000-0000-0000-0000-000000000000") -> Dict[str, Any]:
    ...
    5. Emailable API verification fallback if Port 25 is blocked/untested.
    ...
    # 7. Emailable API Fallback (Triggered when Port 25 Direct SMTP Handshake fails or is restricted)
    logger.info(f"[EMAIL VERIFIER] Port 25 SMTP handshake unavailable/untested for '{email_clean}'. Invoking Emailable API fallback...")
    api_res = await verify_email_with_emailable(email_clean)
